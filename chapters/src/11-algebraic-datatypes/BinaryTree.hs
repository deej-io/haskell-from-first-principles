...
foldTree f acc (Node left a right) =
  f a (foldTree f (foldTree f acc right) left)


foldTestTree :: BinaryTree Int
foldTestTree = foldr insert' Leaf [1..5]

testFoldTree :: IO ()
testFoldTree =
  if foldTree (*) 1 foldTestTree == foldr (*) 1 [1..5]
  then putStrLn "foldTree works!"
  else putStrLn "foldTree dones't work!"
